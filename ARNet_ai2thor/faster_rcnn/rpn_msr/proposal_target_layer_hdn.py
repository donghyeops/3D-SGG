...
def proposal_target_layer(object_rois, gt_objects, gt_relationships,
                n_classes_obj, is_training, graph_generation=False,
    ...
    if is_training:
        ...
        object_labels, object_rois, bbox_targets, bbox_inside_weights, \
            phrase_labels, phrase_rois, \
                mat_object, mat_phrase, sub_spatial_f, obj_spatial_f, sub_label, obj_label \
                    = _sample_rois(object_rois, \
                        gt_objects, gt_relationships, 1, n_classes_obj, is_training, refiner_name, spatial_name)
        ...
        object_rois, phrase_rois, mat_object, mat_phrase, sub_spatial_f, obj_spatial_f, sub_label, obj_label  = \
                    _setup_connection(object_rois, gt_objects, graph_generation=graph_generation, refiner_name=refiner_name, spatial_name=spatial_name)
    ...
    return object_labels, object_rois, bbox_targets, bbox_inside_weights, bbox_outside_weights, mat_object, \
            ...
            sub_spatial_f, obj_spatial_f, sub_label, obj_label ## 추가
...
def _sample_rois(object_rois, gt_objects, gt_relationships, num_images, num_classes, is_training, refiner_name, spatial_name):
...
#### prepare relationships targets
    ...
    for i in range(n_object):
        for j in range(n_object):
            ...
            sub_list.append(i)
            obj_list.append(j)
            phrase_labels.append(pair_labels[i*n_object + j])
    phrase_labels = np.array(phrase_labels)
    sub_list = np.array(sub_list)
    obj_list = np.array(obj_list)
    sub_assignment = sub_list.copy()
    obj_assignment = obj_list.copy()
    '''
    print('-'*10)
    print('-objects-')
    print(n_object)
    print('-phrase_labels-')
    pprint(phrase_labels)
    print('-sub_list-')
    pprint(sub_list)
    print('-obj_list-')
    pprint(obj_list)
    ...
    sub_assignment = npr.choice(list(range(keep_inds.size)), size=rel_bg_num, replace=True)
    obj_assignment = npr.choice(list(range(keep_inds.size)), size=rel_bg_num, replace=True)
    ...
    pprint(sub_assignment)
    print(sub_list == sub_assignment)
    print(obj_list == obj_assignment)
    raise Exception("test")
    '''
    if len(sub_list) == 0:
        phrase_rois, mat_object, mat_phrase = None, None, None
        sub_spatial_f, obj_spatial_f, sub_label, obj_label = None, None, None, None
        return labels, rois, bbox_targets, bbox_inside_weights, \
               phrase_labels, phrase_rois, \
               mat_object, mat_phrase, \
               sub_spatial_f, obj_spatial_f, sub_label, obj_label  ## 추가
    phrase_rois = box_union(object_rois[sub_list, :], object_rois[obj_list, :])
    sub_label = labels[sub_list].copy()
    obj_label = labels[obj_list].copy()
    ...
    if spatial_name == 'bbox':
        ...
        sub_spatial_f = normal_sub_box[:, 1:].astype('float64')
        normal_obj_box[:, 3:] = normal_obj_box[:, 3:] - normal_obj_box[:, 1:3]
        normal_obj_box[:, 1:3] += normal_obj_box[:, 3:]/2
        obj_spatial_f = normal_obj_box[:, 1:].astype('float64')
        ...
        sub_box = object_rois[sub_list, :].copy() ## sub box 좌표 모음 (0, xmin, ymin, xmax, ymax)
...
def _setup_connection(object_rois, gt_objects, graph_generation=False, refiner_name='None', spatial_name='None'):
    """Generate a random sample of RoIs comprising foreground and background
    examples.
    """
    # overlaps: (rois x gt_boxes)
    ...
    if len(object_rois) <= 1:
        phrase_rois, mat_object, mat_phrase = None, None, None
        sub_spatial_f, obj_spatial_f, sub_label, obj_label = None, None, None, None
        return rois, phrase_rois, mat_object, mat_phrase, \
               sub_spatial_f, obj_spatial_f, sub_label, obj_label  ## 추가
    sub_label = labels[id_i].copy()
    obj_label = labels[id_j].copy()
